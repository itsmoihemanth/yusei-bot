...
def add_to_blacklist(json_key: str,object_id: int) -> None:
    """
    ...
    with open("blacklist.json", "r+") as file:
        ...
        file_data[json_key].append(object_id)
    with open("blacklist.json", "w") as file:
        file.seek(0)
        json.dump(file_data, file, indent=4)


def remove_from_blacklist(json_key: str,object_id: int) -> None:
    """
    ...
    with open("blacklist.json", "r") as file:
        file_data = json.load(file)
        file_data[json_key].remove(object_id)
    with open("blacklist.json", "w") as file:
        file.seek(0)
        json.dump(file_data, file, indent=4)